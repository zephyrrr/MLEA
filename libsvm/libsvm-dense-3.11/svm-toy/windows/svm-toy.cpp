...
void button_run_clicked()
{
		...
		param.svm_type == NU_SVR)
	{
		...
		{
			//x_space[i].dim = 1;
			//x_space[i].values = new double[1];
			//x_space[i].values[0] = q->x;
			x_space[i].dim = 2;
			x_space[i].values = new double[2];
			x_space[i].values[0] = 0;
			x_space[i].values[1] = q->x;
		...
		svm_node x;
		x.dim = 2;
		x.values = new double[2];
		int *j = new int[XLEN];

		for (i = 0; i < XLEN; i++)
		{
			x.values[0] = 0;
			x.values[1] = (double) i / XLEN;
	...
	else
	{
		...
		for (i = 0; i < XLEN; i++)
			for (j = 0; j < YLEN; j++) {
				x.values[0] = (double) i / XLEN;
				x.values[1] = (double) j / YLEN;
