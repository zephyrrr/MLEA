...
double binary_class_cross_validation(const problem *prob, const parameter *param, int nr_fold)
{
	...
	{
		dec_values.resize(prob->l);
		ty.resize(prob->l);

		struct model *model = train(prob,param);

		labels = Malloc(int, get_nr_class(model));
		get_labels(model, labels);

		if(get_nr_class(model) > 2) 
		{
			fprintf(stderr,"Error: the number of class is not equal to 2\n");
			exit(-1);
		}
		...
		{
			predict_values(model,prob->x[j], &dec_values[j]);
			ty[j] = (prob->y[j] > 0)? 1: -1;
		}
		if(labels[0] <= 0) 
		{
...
void binary_class_predict(FILE *input, FILE *output){
	...
	//int svm_type=get_svm_type(model);
	//
	//if (svm_type==NU_SVR || svm_type==EPSILON_SVR){
	//	fprintf(stderr, "wrong svm type.");
	//	exit(1);
