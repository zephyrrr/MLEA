...
void l2r_l2_svc_fun::Xv(double *v, double *Xv)
{
	...
	for(i=0;i<l;i++)
	{
		...
		while(xi->index!= -1)
		{
			tmp_value = 0;
			xj = xi + 1;
			double *v_x = &v[((xi->index)*(2*n-xi->index+1))/2];
			while(xj->index!=-1)	// quadratic
			{
				tmp_value += v_x[xj->index]*(xj->value);
				xj++;
			}
			tmp_value *= sqrt2_g;
			tmp_value += v_x[xi->index]*(xi->value)*gamma;
			tmp_value += v[xi->index]*sqrt2_coef0_g;
			Xv[i] += tmp_value*(xi->value);
			xi++;
		}
...
void l2r_l2_svc_fun::subXTv(double *v, double *XTv)
{
	...
	for(i=0;i<sizeI;i++)
	{
		...
		while(xi->index!= -1)
		{
			tmp_value = 0;
			xj = xi + 1;
			double *XTv_x = &XTv[((xi->index)*(2*n-xi->index+1))/2];
			while(xj->index!=-1)	// quadratic
			{
				XTv_x[xj->index] += v[i]*(xj->value)*sqrt2_g*xi->value;
				xj++;
			}			
			XTv_x[xi->index] += v[i]*xi->value*xi->value*gamma;
			XTv[xi->index] += v[i]*sqrt2_coef0_g*xi->value;			
			xi++;
		}
	}
#else
	...
	while (iter < max_iter)
	{
		...
		for (s=0; s<active_size; s++)
		{
			...
			while(xi->index!= -1)
			{
				tmp_value = 0;
				xj = xi + 1;
				double *w_x = &w[((xi->index)*(2*n-xi->index+1))/2];
				while(xj->index!=-1)	// quadratic
				{
					tmp_value += w_x[xj->index]*(xj->value);
					xj++;
				}
				tmp_value *= sqrt2_g;
				tmp_value += w_x[xi->index]*(xi->value)*gamma;
				tmp_value += w[xi->index]*sqrt2_coef0_g;
				G += tmp_value*(xi->value);
				xi++;
			}
#else
			while(xi->index!= -1)
			...
			if(fabs(PG) > 1.0e-12)
			{
				...
				while (xi->index != -1)
				{
					tmp_value = d*xi->value;
					xj = xi + 1;
					double *w_x = &w[((xi->index)*(2*n-xi->index+1))/2];
					w_x[xi->index] += gamma*tmp_value*(xi->value);
					while(xj->index != -1)	// quadratic
					{
					    w_x[xj->index] += sqrt2_g*tmp_value*(xj->value);
					    xj++;
					}
					w[xi->index] += tmp_value*sqrt2_coef0_g;
					xi++;
				}
#else
				while (xi->index != -1)
...
#ifdef POLY2
		subprob.coef0 = prob->coef0;
		subprob.gamma = prob->gamma;
#endif
...
const char *check_parameter(const problem *prob, const parameter *param)
{
	...
	if(param->solver_type != L2R_L2LOSS_SVC_DUAL
		...
		return "Currently only support L2-regularized L1- and L2-loss SVC";
#endif
