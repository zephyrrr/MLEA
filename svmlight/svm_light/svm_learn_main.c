...
void   _svm_learn_read_input_parameters(int, char **, char *, char *, char *, long *, 
			     LEARN_PARM *, KERNEL_PARM *);
void   _svm_learn_wait_any_key();
void   _svm_learn_print_help();
...
{  
  ...
  _svm_learn_read_input_parameters(argc,argv,docfile,modelfile,restartfile,&verbosity,
			&learn_parm,&kernel_parm);
...
void _svm_learn_read_input_parameters(int argc,char *argv[],char *docfile,char *modelfile,
			   ...
			   LEARN_PARM *learn_parm,KERNEL_PARM *kernel_parm)
{
  ...
  for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
    switch ((argv[i])[1]) 
      { 
      case '?': _svm_learn_print_help(); exit(0);
      ...
      default: printf("\nUnrecognized option %s!\n\n",argv[i]);
	       _svm_learn_print_help();
	       exit(0);
      }
  }
  if(i>=argc) {
    printf("\nNot enough input parameters!\n\n");
    _svm_learn_wait_any_key();
    _svm_learn_print_help();
    exit(0);
  }
