...
void   _svm_struct_learn_read_input_parameters(int, char **, char *, char *,long *, long *,
			     STRUCT_LEARN_PARM *, LEARN_PARM *, KERNEL_PARM *,
			     int *);
void   _svm_struct_learn_wait_any_key();
void   _svm_struct_learn_print_help();
...
void _svm_struct_learn_read_input_parameters(int argc,char *argv[],char *trainfile,
			   char *modelfile,
			   ...
			   int *alg_type)
{
  ...
  for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
    switch ((argv[i])[1]) 
      { 
      case '?': _svm_struct_learn_print_help(); exit(0);
      ...
      default: printf("\nUnrecognized option %s!\n\n",argv[i]);
	       _svm_struct_learn_print_help();
	       exit(0);
      }
  }
  if(i>=argc) {
    printf("\nNot enough input parameters!\n\n");
    _svm_struct_learn_wait_any_key();
    _svm_struct_learn_print_help();
    exit(0);
  }
