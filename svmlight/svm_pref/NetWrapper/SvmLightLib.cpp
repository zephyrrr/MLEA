...
SVMLIGHTLIB_API void MulticlassClassify(int model_id, int feature_vector_count, int *feature_vectors)
{
	...
	for (int i = 0; i < test_sample.n; i++)
	{
		...
		feature_vector->first->m_num_scores = y.totdoc;
		delete[] feature_vector->first->m_scores; 
		feature_vector->first->m_scores = new double[y.totdoc];
		memcpy(feature_vector->first->m_scores, &y._class[1], y.totdoc * sizeof(double));
		delete[] y._class;
	}
	for (int i = 0; i < test_sample.n; i++)
	{
