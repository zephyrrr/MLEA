...
void _svm_classify_read_input_parameters(int, char **, char *, char *, char *, long *, 
			   long *);
void _svm_classify_print_help(void);
...
int _svm_classify (int argc, char* argv[])
{
  ...
  _svm_classify_read_input_parameters(argc,argv,docfile,modelfile,predictionsfile,
...
void _svm_classify_read_input_parameters(int argc, char **argv, char *docfile, 
			   ...
			   long int *verbosity, long int *pred_format)
{
  ...
  for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
    switch ((argv[i])[1]) 
      { 
      case 'h': _svm_classify_print_help(); exit(0);
      case 'v': i++; (*verbosity)=atol(argv[i]); break;
      case 'f': i++; (*pred_format)=atol(argv[i]); break;
      default: printf("\nUnrecognized option %s!\n\n",argv[i]);
	       _svm_classify_print_help();
	       exit(0);
      }
  }
  if((i+1)>=argc) {
