...
SAMPLE      read_struct_examples(char *file, STRUCT_LEARN_PARM *sparm)
{
  ...
  for(i=0;i<sample.examples[0].x.totdoc;i++) {
    length=1;
    if(sample.examples[0].y._class[i]>0) 
      nump++;
    else 
      numn++;
  ...
  if((sparm->loss_function == ZEROONE) 
    ...
    for(i=0;i<sample.examples[0].x.totdoc;i++) {
      if(sample.examples[0].y._class[i]>0)
	sample.examples[0].y._class[i]=0.5*100.0/(numn+nump);
      else
	sample.examples[0].y._class[i]=-0.5*100.0/(numn+nump);
    }
  }
  ...
  if(sparm->loss_function == SWAPPEDPAIRS) {
    for(i=0;i<sample.examples[0].x.totdoc;i++) {
      /*      if(sample.examples[0].y._class[i]>0)
	sample.examples[0].y._class[i]=numn*0.5;
      else
      sample.examples[0].y._class[i]=-nump*0.5; */
      if(sample.examples[0].y._class[i]>0)
	sample.examples[0].y._class[i]=0.5*100.0/nump;
      else
	sample.examples[0].y._class[i]=-0.5*100.0/numn;
    }
  }
  if(sparm->loss_function == AVGPREC) {
    for(i=0;i<sample.examples[0].x.totdoc;i++) {
      if(sample.examples[0].y._class[i]>0)
	sample.examples[0].y._class[i]=numn;
      else
	sample.examples[0].y._class[i]=-nump;
    }
  }
						    ...
						    int loss_type)
{
  ...
  for(i=0;i<x.totdoc;i++) {
    ...
    diff=loss_step-2.0*y._class[i]*score[i];
    if(diff > 0) {
      ybar._class[i]=-y._class[i];
      valmax+=loss_step;
    }
    else {
      ybar._class[i]=y._class[i];
    }
    valmax+=ybar._class[i]*score[i];
  }
  ...
  if(struct_verbosity >= 2) {
    ...
    fy=psi(x,y,sm,sparm);
    fybar=psi(x,ybar,sm,sparm);
    exy=create_example(0,0,1,1,fy);
    exybar=create_example(0,0,1,1,fybar);
						     ...
						     int loss_type)
{
  ...
  numn=0;
  for(i=0;i<x.totdoc;i++) {
    score[i]=fabs(y._class[i])*classify_example(&svm_model,x.doc[i]);
    if(y._class[i] > 0) {
  ...
  /*  for(i=0;i<nump;i++) {
    if(i<threshp) 
      ybar._class[scorep[i].id]=1;
    else 
      ybar._class[scorep[i].id]=-1;
  }
  for(i=0;i<numn;i++) {
    if(i<threshn) 
      ybar._class[scoren[i].id]=-1;
    else 
      ybar._class[scoren[i].id]=1;
      } */
  for(i=0;i<nump;i++) {
    if(i<threshp) 
      ybar._class[scorep[i].id]=y._class[scorep[i].id];
    else 
      ybar._class[scorep[i].id]=-y._class[scorep[i].id];
  }
  for(i=0;i<numn;i++) {
    if(i<threshn) 
      ybar._class[scoren[i].id]=y._class[scoren[i].id];
    else 
      ybar._class[scoren[i].id]=-y._class[scoren[i].id];
  }
						     ...
						     int loss_type)
{
  ...
  if(sparm->loss_function == SWAPPEDPAIRS) { /* number of swapped pairs (ie. ROC Area) */
    ...
    for(i=0;i<numn+nump;i++) {
      if(y._class[predset[i].id] > 0) {
	ybar._class[predset[i].id]=((numn-2.0*sumn)*0.5*100.0/nump)/numn;
	sump++;
      }
      else {
	ybar._class[predset[i].id]=-((nump-2.0*(nump-sump))*0.5*100.0/nump)/numn;
	sumn++;
      }
    }
  }
						     ...
						     int loss_type)
{
  ...
  if(sparm->loss_function == AVGPREC) { /* Average Precision */
    ...
    */
    ...
    for(i=0;i<nump;i++) { 
      ybar._class[scorep[i].id]=numn-2*negabove[i];
      for(j=0;j<negabove[i];j++) {
	ybar._class[scoren[j].id]++;
      }
		 ...
		 STRUCT_LEARN_PARM *sparm)
{
  ...
  if(sm->svm_model->kernel_parm.kernel_type == LINEAR) {
    ...
    for(i=0;i<y.totdoc;i++) 
      add_vector_ns(sum,x.doc[i]->fvec,y._class[i]);
...
double rocarea(LABEL y, LABEL ybar)
{
  ...
  for(i=0;i<ybar.totdoc;i++) {
    predset[i].score=ybar._class[i];
    predset[i].tiebreak=-y._class[i];
    predset[i].id=i;
  }
...
double avgprec_compressed(LABEL y, LABEL ybar)
{
  ...
  for(i=0;i<ybar.totdoc;i++) {
    if(y._class[i] > 0) {
      predset[ii].score=ybar._class[i];
      predset[ii].tiebreak=-y._class[i];
      predset[ii].id=i;
      ii++;
    }
  }
