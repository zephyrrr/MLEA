...
#include <ZigzagPattern_oldest.mqh>
#include <Utils.mqh>
...
int OnInit()
{
    ...
    m_indicatorZigzag=iCustom(_Symbol,_Period,"Examples\\ZigzagColor");
...
#define ZIGZAG_INDICATOR_COUNT 2500
//+------------------------------------------------------------------+
...
void OnTick()
{
    ...
    double zigzagValuesHigh[ZIGZAG_INDICATOR_COUNT];
    double zigzagValuesLow[ZIGZAG_INDICATOR_COUNT];
    ArraySetAsSeries(zigzagValuesHigh, true);
    ArraySetAsSeries(zigzagValuesHigh, true);
    
    if(CopyBuffer(m_indicatorZigzag, 0, 0, ZIGZAG_INDICATOR_COUNT, zigzagValuesHigh) < 0)
    {
        Print("Error copying Zigzag indicator buffer - error:", GetLastError());
        return;
    }
    if(CopyBuffer(m_indicatorZigzag, 1, 0, ZIGZAG_INDICATOR_COUNT, zigzagValuesLow) < 0)
    {
        Print("Error copying Zigzag indicator buffer - error:", GetLastError());
        return;
    }
    ...
    for(int i=0; i<ZIGZAG_INDICATOR_COUNT; ++i)
    {
        if (nowIsHigh == -1)
        {
            if (zigzagValuesHigh[i] != 0)
            {
                m_zigzagValues[ZIGZAG_INDICATOR_COUNT - 1 - i] = zigzagValuesLow[i];
                nowIsHigh = 0;
            }
            else if (zigzagValuesLow[i] != 0)
            {
                m_zigzagValues[ZIGZAG_INDICATOR_COUNT - 1 - i] = zigzagValuesHigh[i];
                nowIsHigh = 1;
            }
            else
            {
                m_zigzagValues[ZIGZAG_INDICATOR_COUNT - 1 - i] = 0;
            }
        }
        else if (nowIsHigh == 1)
        {
...
bool LoadTxtDeal()
{
    ...
    while(true)
    {
        ...
        if (s != NULL && StringLen(s) > 0)
        {
            ...
            //if (closeTime > D'2007.01.01' || closeTime < D'2006.01.01')
...
{  
    ...
    double zigzagValuesLatest[2 * maxPatternCnt];
    string p = GetZigzagPatternLatest(m_zigzagValues, mqlTick.bid, 12, zigzagValuesLatest);
    
    //if (StringSubstr(TimeToString(TimeCurrent()), 0, 16) == "2009.01.08 14:54")
    //{
        //Print(p + "," + m_lastZigzag);
    //}
    ...
    //if (m_lastZigzag != NULL)
    {
        ...
        comment += ", B:" + DoubleToString(t1, 2) + ", " + DoubleToString(ta, 1) + ", " + DoubleToString(tb, 1);
        ...
        comment += ", S:" + DoubleToString(t2, 2) + ", " + DoubleToString(ta, 1) + ", " + DoubleToString(tb, 1);
    ...
    //if (TimeCurrent() == D'2009.01.08 14:54')
    //{
        //for(int i=0; i<50; ++i)
