...
bool LoadTxtDeal()
{
    ...
    while(true)
    {
        ...
        if (s != NULL && StringLen(s) > 0)
        {
            ...
            {
                //Print("Loaded History Idx is ", m_historyDealsTxtIdx);
...
bool LoadTxtDeal_new()
{
    ...
    while(true)
    {
        ...
        if (s != NULL && StringLen(s) > 0)
        {
            ...
            if (closeTime >= D'2000.01.01' 
                &&(!(bool)MQL5InfoInteger(MQL5_TESTING) || closeTime < TimeCurrent()))
            {
                //int i1 = StringSubstr(s1, 0, 1) == "H" ? 0 : 1;
                //string i2s = StringSubstr(s1, 2, StringSubstr(s1, 3, 1) == "," ? 1 : 2);
                //string i3s = StringSubstr(s1, 2 + StringLen(i2s) + 1);
...
{  
    ...
    //Print("Now History Idx is ", m_historyDealsTxtIdx);
