...
//--- if an object is being dragged, pass control to the special drag object
   if(m_drag_object!= NULL && m_drag_object.OnEvent(id,lparam,dparam,sparam)) return(true);
   ...
   for(int i=total-1;i>=0;i--)
     {
      ...
      if(control.OnEvent(id,lparam,dparam,sparam)) return(true);
     }
//--- not handled
   return(false);
  }
...
//--- if an object is being dragged, pass control to the special drag object
   if(m_drag_object!= NULL && m_drag_object.OnMouseEvent(x,y,flags)) return(true);
//--- loop by elements of group
   int total=m_controls.Total();
   for(int i=total-1;i>=0;i--)
     {
      ...
      //--- check of pointer
      if(control==NULL) continue;
      if(control.OnMouseEvent(x,y,flags))  return(true);
     }
...
//--- correct visibility of the added control if needed
...
//--- loop by elements of group
   ...
   for(int i=0;i<total;i++)
     ...
     //--- delete item from group
      if(pointer==control)
        {
         if(!m_controls.Delete(i)) return(false);
         return(true);
        }
     }
//--- failure
   ...
   for(int i=0;i<total;i++)
     {
      ...
      //--- check of pointer
      if(control==NULL) continue;
      if(!control.Align(Rect()))  return(false);
     }
//--- handled
   return(true);
  }
