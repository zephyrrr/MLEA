...
//--- create
   if(!m_button.Create(m_chart_id,m_name+"Button",m_subwin,x1,y1,x2,y2))          return(false);
   if(!m_button.BmpNames("::res\\RadioButtonOff.bmp","::res\\RadioButtonOn.bmp")) return(false);
...
//--- create
   ...
   if(!m_label.Text(m_name))                                                      return(false);
   if(!m_label.Color(Black))                                                      return(false);
...
//--- this is due to that radio button can not be turned off by clicking on it (it can be only turned on)
   if(m_button.Pressed())   return(true);
//--- turn on the radio button
