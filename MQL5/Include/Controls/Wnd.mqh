...
CWnd* CWnd::ControlFind(const long id)
  {
   CWnd* result=NULL;
//--- check
   if(id==m_id) result=GetPointer(this);
//--- return the result
   return(result);
  }
//+------------------------------------------------------------------+
...
bool CWnd::OnMouseDown(void)
  {
   if(IS_CAN_DRAG)         return(OnDragStart());
...
bool CDragWnd::OnDragProcess(const int x,const int y)
  {
   ...
   int dy=y-m_mouse_y;
//--- check shift
   if(Left()+dx<m_limit_left)     dx=m_limit_left-Left();
   if(Top()+dy<m_limit_top)       dy=m_limit_top-Top();
   if(Right()+dx>m_limit_right)   dx=m_limit_right-Right();
   if(Bottom()+dy>m_limit_bottom) dy=m_limit_bottom-Bottom();
//--- shift
   Shift(dx,dy);
//--- save
   m_mouse_x=x;
