...
bool CChartObjectStdDevChannel::Create(long chart_id,const string name,const int window,
                                       const datetime time1,const datetime time2,const double deviation)
  {
   if(!ObjectCreate(chart_id,name,OBJ_STDDEVCHANNEL,window,time1,0.0,time2,0.0)) return(false);
   if(!Attach(chart_id,name,window,2))                                           return(false);
   if(!Deviations(deviation))                                                    return(false);
//--- succeed
   return(true);
  }
//+------------------------------------------------------------------+
...
//--- checking
   ...
   if(m_chart_id==-1)                        return(false);
//--- writing
   if(!CChartObjectTrend::Save(file_handle)) return(false);
...
//--- checking
   ...
   if(m_chart_id==-1)                        return(false);
//--- reading
   if(!CChartObjectTrend::Load(file_handle)) return(false);
...
bool CChartObjectRegression::Create(long chart_id,const string name,const int window,
                                    const datetime time1,const datetime time2)
  {
   if(!ObjectCreate(chart_id,name,OBJ_REGRESSION,window,time1,0.0,time2,0.0)) return(false);
   if(!Attach(chart_id,name,window,2))                                        return(false);
//--- succeed
   return(true);
  }
...
bool CChartObjectPitchfork::Create(long chart_id,const string name,const int window,
                                   ...
                                   const datetime time3,const double price3)
  {
   if(!ObjectCreate(chart_id,name,OBJ_PITCHFORK,window,time1,price1,time2,price2,time3,price3)) return(false);
   if(!Attach(chart_id,name,window,3))                                                          return(false);
//--- succeed
   return(true);
  }
